...
config = [('AE_64', '', './storage/non_pretrained/checkpoints/Pointnav-RoboTHOR-Vanilla-RGB-ResNet-DDPPO/custom/2021-08-11_17-04-24/exp_Pointnav-RoboTHOR-Vanilla-RGB-ResNet-DDPPO_custom__stage_00__steps_000030005610.pt', '2021-08-11_17-04-24', 'NoPretrain_64')]
# [('VAE_64', './domain_adaptation/checkpoints/VAE/weather/best_model.pt', './storage/vae/checkpoints/Pointnav-RoboTHOR-Vanilla-RGB-ResNet-DDPPO/custom/2021-08-09_06-17-30/exp_Pointnav-RoboTHOR-Vanilla-RGB-ResNet-DDPPO_custom__stage_00__steps_000030005760.pt', '2021-08-09_06-17-30', 'VAE_64'),
...
#             command = f'python main.py -o=storage/{tensorboard_name} -b=projects/robustnav_baselines/experiments/robustnav_eval {task}_robothor_vanilla_rgb_custom_ddppo -s=12345 -et=custom -em={model} -emb={model_base} -las={latent_size} -c={rn_ckpt} -t={date}'
...
task_corrupt = ['cam_crack'] # ['cam_crack', 'fov']

for model, model_ckpt, rn_ckpt, date, name in config:
    for c in task_corrupt:
        tensorboard_name = f'{name}_{c}'        
        command = f'python main.py -o=storage/{tensorboard_name} -b=projects/robustnav_baselines/experiments/robustnav_eval {task}_robothor_vanilla_rgb_custom_ddppo_{c} -s=12345 -et=custom -em={model} -emb={model_base} -las={latent_size} -c={rn_ckpt} -t={date}'
